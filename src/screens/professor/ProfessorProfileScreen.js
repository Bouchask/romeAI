...
import { PROFESSORS, MODULES } from '../../data/mockData';
...
export default function ProfessorProfileScreen() {
  ...
  return (
    <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
      <ScreenHeader 
        title="My Profile" 
        subtitle="Faculty Account Settings" 
      />
      ...
      <View style={styles.content}>
        ...
        <Card noPadding>
          <ProfileRow icon="mail-outline" label="University Email" value={user?.email || 'professor@university.edu'} />
          <ProfileRow icon="id-card-outline" label="Faculty ID" value={`FAC-2024-${user?.id || '001'}`} />
          <ProfileRow icon="calendar-outline" label="Department" value="Computer Science" isLast />
        </Card>

        <Text style={styles.sectionTitle}>Modules Taught</Text>
        <Card noPadding>
          {profData.modules.map((modId, index) => {
            const mod = MODULES.find(m => m.id === modId);
            return (
              <View key={modId} style={[styles.moduleRow, index === profData.modules.length - 1 && styles.lastRow]}>
                <View style={styles.moduleIcon}>
                  <Ionicons name="book" size={18} color={theme.colors.primary} />
                </View>
                <View style={styles.moduleBody}>
                  <Text style={styles.moduleName}>{mod?.name}</Text>
                  <Text style={styles.moduleCode}>Module ID: {modId.toUpperCase()}</Text>
                </View>
              </View>
            );
          })}
...
const styles = StyleSheet.create({
  ...
  moduleRow: { flexDirection: 'row', alignItems: 'center', padding: theme.spacing.md, borderBottomWidth: 1, borderBottomColor: theme.colors.border },
  ...
  moduleBody: { flex: 1 },
  moduleName: { fontSize: 15, fontWeight: '600', color: theme.colors.text },
  moduleCode: { fontSize: 12, color: theme.colors.textMuted, marginTop: 2 },
